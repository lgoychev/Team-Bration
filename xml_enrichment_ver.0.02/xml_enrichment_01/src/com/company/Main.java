...
//SOURCE and OUTPUT file are locate in com.company folder, don't move
    public static final String inputXmlFilePath = ("SOURCE.xml"); //define a Source name: SOURCE.xml (is correct); SOURCE_01.xml (no contract number, missin "-"); SOURCE_02.xml (POnumber tag not exist)
    ...
    public static void main(String argv[])
        ...
        if (poNumber != null && !isEmpty(poNumber.getTextContent())) {
            ...
            if (valueToSplit.contains("-")) {
                ...
                JOptionPane.showMessageDialog(null, "The XML File was successfully enriched!", "Message",
                  ...
                  JOptionPane.showMessageDialog(null, "No contract number available in the PONumber!", "Source file error",
                          JOptionPane.INFORMATION_MESSAGE); //Pop-up Dialog window which present an error message
            }
        } else {
            JOptionPane.showMessageDialog(frame, "PONumber tag is empty or does not exist!", "Source file error",
                    JOptionPane.INFORMATION_MESSAGE); //Pop-up Dioalog window which present an error message
        }
    }
